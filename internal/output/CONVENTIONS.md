...
- Optional identity metadata: configs may provide non-avatar `identity.display_name`, `identity.role`, and `identity.affiliation` text. The durable transcript cast is the primary replay identity; never hide, replace, abbreviate, or title-case the exact `agent_id` inside the badge. Old configs and transcripts require no identity fields.
- Agent visual treatment: each agent gets one stable theme-adaptive ANSI palette slot from config order, persisted in transcript cast metadata. The same accent applies to that agent's badge, turn marker, and per-agent stats row. Color is decorative; the ordinal and label carry identity.
